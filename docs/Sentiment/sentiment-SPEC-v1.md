...
### 2.3 `news_reactions`
Jeden řádek per `(event_id FK, symbol)` (široký tvar od #998, ADR-0031; do té doby řádek per okno). Per okno `w ∈ {1,5,15,60} ∪ {1440,2880,7200,14400}` sloupce `ret_w`, `range_w`, `cont_w` a u minutových oken `vol_z_w`; per fázi (minutová `_min` / denní `_daily`) sloupce `deferred_*`, `regime_*` (GEX režim v čase eventu, #402) a `computed_at_*`. Nezměřené okno (chybí bar) = NULL v celé čtveřici. Významy: `ret_bp` = změna ceny v bps od ts_event do konce okna, `range_bp` = high−low okna, `vol_z` = objem okna vs. průměr stejné denní minuty (z-score, 5.1; denní okna ho nemají). `contaminated` = do okna spadl jiný event s importance ≥ 2 (5.1). `deferred` = trhy byly v čase eventu zavřené, okno se měří od prvního obchodovaného baru (5.1). API (`GET /news/{id}`) reakce vrací dál jako řádky per (symbol, okno).
...
- **Denní okna (rev. 2026-08-12, #564):** navíc horizonty **1/2/5/10 obchodních dní** — referenční efekt zprávy žije dny až dva týdny a minutová okna ho nevidí (měření #563). Uložené jako `window_min = N × 1440` (identifikátor, ne doslovné minuty). Definice: 1d = nejbližší settle po události, Nd = o N−1 Globex seancí dál (seance = existence barů, ADR-0023). Základní cena stejná jako u minutových oken (poslední bar před událostí, gap u zavřeného trhu v `ret_bp`). Vědomé odchylky: `contaminated` je u denních oken vždy false (v denním horizontu spadne do okna prakticky vždy jiný event — denní buckety měří režimovou odpověď kategorie, ne izolovanou zprávu), `vol_z` = NULL, `range_bp` ze session high/low zúčastněných seancí (první zahrnuje i pohyb před událostí). Zapisují se najednou až po uzavření nejdelšího okna. Které okno řídí signály, zůstává rozhodnutí #453 nad nasbíranými statistikami — primární okno se touto revizí nemění.
...
- **Stav (state):** `RiskOn / RiskOff / Neutral`. **Výchozí pravidla jsou pinnutá zde** (konfig je jen override, jinak nejdou psát golden testy). *(Rev. 2026-08-12, #563 — původní pravidlo „směr vlny ∧ hloubka ≥ potvrzovací práh" drželo Neutral 96,5 % dní a signální větev spala; nahrazeno polohou dle referenčního přístupu.)* **Stav = poloha denního close vůči oběma průměrům, definovaný každý den:** RiskOn ⇔ close nad MA5 i MA10; RiskOff ⇔ pod oběma; mezi průměry Neutral. **Polarita trendu = MA5 vs. MA10** (up/down) je atribut stavu. Hloubka aktuální vlny a potvrzovací práh (průměrná hloubka historických vln opačného směru ze `sentiment_waves`) se dál počítají a reportují jako **atributy** — stav už negatují. **Čtení stavu:** názvy popisují náladu (jak na trh dopadají zprávy), ne predikci ceny — měření #563 (2024–2026) ukázalo kontrariánské následné výnosy (RiskOff období se vykupovala); směr smí určit jen kalibrace signálů nad track recordem (6.2/7.3), nikdy název stavu.
...
- **Korekční epizody — pokus vs. negace** *(rev. 2026-09-14, #565, ADR-0037; vrstva VEDLE vln a stavu, ne náhrada)*. Vlna trvá Ø 1,5 dne, takže korekci nálady měří samostatná epizoda nad denním `close_z` (close / σ(100 seancí), #640): **start** = pokles close_z pod klouzavé 20denní maximum o ≥ D σ (další start až po odjištění — drawdown < D — a nikdy uvnitř probíhající epizody); **pokus** = close_z zpět nad maximem z dne startu do H obchodních dní (po–pá, svátky se negatují); **negace** = jinak, rozhodnutí = H-tý obchodní den. Hloubka = největší pokles pod referenční úroveň (σ). Zahlazení „nad úroveň startu / nad MA10" se **nepoužívá** — při měření dávalo 0 negací. Parametry jsou **verzovaný placeholder** (`params_version` 1: D = 1 σ, H = 10, okno 20 — první měření nad živou érou σ kontaminovanou backfillem D nerozlišilo; přeměření ~11/2026, ≥ 20 rozhodnutých epizod per symbol); změna = nová verze + full-replace tabulky `sentiment_episodes` (plní WavesJob, jen uzavřené dny). Stav: `open` (probíhá, třída neznámá) / `attempt` (zahlazeno dnes) / `negation` (trvá bez návratu nad referenční úroveň) / `none`; `correction_threshold` = 20denní maximum − D v σ. UI: badge v chipu stavu, linie prahu ve spodním panelu Sentiment (Daily), blok ve Stats — všude označeno „předběžné". Kalibrace výhradně nad živou érou (#640); vztah k ceně měření nepotvrdilo, epizoda popisuje náladu.
...
Signály se generují až po nasbírání minima dat: bucket eventu musí mít `n ≥ n_min` (default **30**) nekontaminovaných měřených reakcí a **Wilson 95% lower bound hit-rate kategorie > 0.50** a **|Ø reakce| ≥ 1 bp** (ADR-0042 — LB měří jen spolehlivost směru, u obřích bucketů projde i nulový efekt) na primárním okně; výsledek nese `news_model_stats.gate_open` (bodová hit-rate 55 % při n=20 je statisticky nerozlišitelná od mince; navíc při desítkách bucketů nějaký „projde" náhodou vždy — proto interval, ne bod). Do té doby UI zobrazuje jen „collecting data: X %" — žádné signály. Gate je per bucket, takže časté kategorie (FED, makro) naběhnou dřív než vzácné. UI u signálu vždy ukazuje n a Wilson LB, ne jen bodové procento.
