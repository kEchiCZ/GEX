...
| `ANTHROPIC_API_KEY` | — | Klíč pro **Vysvětlení zprávy** (#1126 3d, v1.18): tlačítko „Vysvětlit" u karty v News volá Claude přes oficiální SDK (api kontejner, přes `env_file`). Bez klíče API vrací 503, cache jede dál |
| `GEXLENS_NEWS_EXPLAIN_ENABLED` | false | Zapíná endpoint `POST /news/{id}/explain`. Jiný účel než Gemini větev: porozumění, ne predikce — do SentIndexu, vah ani signálů nic neteče (R4). Odpovědi navždy v PG `news_explanations` (event_id, model, text, tokeny, created_at) |
| `GEXLENS_NEWS_EXPLAIN_MODEL` | claude-opus-5 | Model; jedno vysvětlení ≈ 600 tokenů (systémový prompt jde přes prompt cache) |
| `GEXLENS_NEWS_EXPLAIN_DAILY_TOKENS` | 300000 | Denní strop input+output tokenů za UTC den, po překročení 429 (0 = bez stropu). Spotřeba je v logu api (`Vysvětlení zprávy N: model, in+out tokenů (dnes X/strop)`) |
