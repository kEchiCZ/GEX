...
| TWS/Gateway spadlo po přihlášení na mobilu | Limit jednoho přihlášení IBKR. Od v1.17 aplikace jede dál z tastytrade (pipeline se založí z posledního známého řetězu, cena, zdi, OI i svíčky z tasty); po opětovném přihlášení v TWS/Gateway se sama vrátí k IBKR. |
| **Přihlásil jsem se na mobilu a data stála** | Market data jsou u IBKR **per uživatel**, takže je mobil přetáhne k sobě (error 10197) — aplikace zůstane připojená, jen jí přestanou chodit ticky. Řeší to sama: do **30 s** převezme cenu a do **3 minut** celý opční řetěz **tastytrade** a v hlavičce se rozsvítí jantarový chip; od v1.17 se od 3. minuty doplňují i **svíčky** (úsek označený jako doplněný) a když mobil shodí i Gateway, pipeline se založí z posledního známého řetězu a tasty (alert „degradovaný start"). Po návratu IBKR se přepne zpátky (cena po 60 s, řetěz po 5 čistých minutách). Dělat nemusíš nic. **Co z tasty není:** opční objem a Prémie $ ve strike profilu, panely Vol, OptVol, CumΔ a Delta flow — stojí do návratu IBKR. |
| **Chip „⤳ řetěz: tastytrade" v hlavičce** | Běží záložní zdroj dat (viz řádek výše). Graf, heatmapa, zdi, GEX úrovně, OI i greeks jedou dál (greeks na strikech, kde je dxFeed neposílá, dopočítává BS model — v řadě zdrojů greeks jako `computed`). **Stojí ale CumΔ a net objem** — tastytrade denní objem ve stejném významu nedodává a vymyslet ho by bylo horší než ho přiznat. Rozjedou se samy po návratu na IBKR. |
